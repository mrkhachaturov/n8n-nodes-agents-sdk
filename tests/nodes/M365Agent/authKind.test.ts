...
describe('M365Agent — authKind param + credential gating', () => {
  ...
  it('credentials declare both types with displayOptions gating', () => {
    ...
    expect(creds.find(c => c.name === 'm365AgentApi')?.displayOptions?.show).toEqual({ authKind: ['classicBot'] });
    expect(creds.find(c => c.name === 'm365Agent365Api')?.displayOptions?.show).toEqual({ authKind: ['agent365'] });
  });
});
