...
describe('message/send routing', () => {
  ...
  it('classic authKind → router receives classicBot', async () => {
    ...
    await sendExecute.call(ctx, 0, 'classicBot' as any, { appType: 'SingleTenant', clientId: 'c' } as any, bundles);
  ...
  it('agent365 + autonomous → router receives downstreamApi=MessagingBotApi', async () => {
    const ctx = makeExecuteMock({
      ...
      nodeParams: (name: string, _i: number, fallback?: unknown) => {
        const map: Record<string, unknown> = {
          ...
          resource: 'message', operation: 'send',
          conversationSource: 'fromEnvelope', messageText: 'hi',
        };
        return map[name] ?? fallback;
      },
    ...
    await sendExecute.call(ctx, 0, 'agent365' as any, {
      tenantId: 't', blueprintAppId: 'bp', transport: 'sidecar',
      sidecarUrl: 'http://s:5000', outboundDownstreamApi: 'MessagingBotApi',
      validateVia: 'sameAsOutbound',
    } as any, new Map());
  ...
  it('uses makeBundleKey to cache — different identityMode produces different key', () => {
    expect(makeBundleKey('https://x/', 'agent365', 'autonomous'))
      .not.toEqual(makeBundleKey('https://x/', 'agent365', 'agentUser', 'u@x.com'));
  });
});
