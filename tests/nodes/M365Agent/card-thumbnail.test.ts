...
describe('M365Agent thumbnailCard send', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockCreate.mockReturnValue(fakeBundle);
		fakeClient.sendToConversation.mockResolvedValue({ id: 'sent-thumb' });
	});

	it('sends a thumbnail attachment and preserves input', async () => {
		const node = new M365Agent();
		const ctx = makeExecuteContext({
			inputItems: [THUMB_ENVELOPE],
			...
			parameters: {
				...
				options: { fallbackText: 'Thumb fallback' },
			},
		});
		...
		expect(activity.attachments[0].contentType).toBe('application/vnd.microsoft.card.thumbnail');
		expect(activity.attachments[0].content.title).toBe('Snippet');
		expect(activity.attachments[0].content.buttons).toHaveLength(1);
		expect(activity.text).toBe('Thumb fallback');
		expect(result[0][0].json).toMatchObject({
			customField: 'preserved',
			cardSendResult: { id: 'sent-thumb' },
		});
	});
});
