...
describe('M365Agent audioCard send', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockCreate.mockReturnValue(fakeBundle);
		fakeClient.sendToConversation.mockResolvedValue({ id: 'sent-audio' });
	});

	it('sends an audio attachment and preserves input', async () => {
		...
		const ctx = makeExecuteContext({
			...
			parameters: {
				...
				buttons: {},
				options: { fallbackText: 'Audio fallback' },
			},
		});
		...
		expect(activity.attachments[0].contentType).toBe('application/vnd.microsoft.card.audio');
		expect(activity.text).toBe('Audio fallback');
		expect(result[0][0].json).toMatchObject({
			customField: 'preserved',
			cardSendResult: { id: 'sent-audio' },
		});
	});
