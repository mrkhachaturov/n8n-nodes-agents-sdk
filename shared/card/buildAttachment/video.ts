...
export function buildVideoAttachment(ctx: IExecuteFunctions, itemIndex: number): Attachment {
	...
	if (media.length === 0) {
		throw new NodeOperationError(ctx.getNode(), 'Video Card requires at least one media URL', {
			itemIndex,
		});
	}
	...
	return CardFactory.videoCard(title, media, buttons.length > 0 ? buttons : undefined, other);
}
